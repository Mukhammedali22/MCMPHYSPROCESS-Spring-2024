...
void Fractional_step_method(double U_old[P][M][N], double U_new[P][M][N], int N, int M, int P, double dx, double dy, 
    ...
    do{
            ...
            for(int j = 1; j < M-1; ++j){
                for(int i = 1; i < N-1; ++i){
                    D[k][j][i] = U_old[k][j][i] / dt
                        + a2*(
                        (U_old[k][j][i+1] - 2*U_old[k][j][i] + U_old[k][j][i-1])
                            / (2*dx*dx)
        ...
        for(int k = 1; k < P-1; ++k){
            for(int j = 1; j < M-1; ++j){
                for(int i = 1; i < N-1; ++i){
                    ...
                    - a2*(U_old[k][j+1][i] - 2*U_old[k][j][i] + U_old[k][j-1][i])
                        / (2*dy*dy);
                }
            }
        }
        ...
        for(int k = 1; k < P-1; ++k){
            for(int j = 1; j < M-1; ++j){
                for(int i = 1; i < N-1; ++i){
                    D[k][j][i] = U_new[k][j][i] / dt
                        - a2*(U_old[k+1][j][i] - 2*U_old[k][j][i] + U_old[k-1][j][i])
                            / (2*dz*dz);
                }
            }
        }
...
int main(){
    ...
    Fractional_step_method(U_old, U_new, N, M, P, dx, dy, dz, dt, a2);
