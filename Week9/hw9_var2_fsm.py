...
@timeit
def Fractional_step_method(U:np.ndarray, N, M, P, dx, dy, dz, dt, 
                           a2=1, eps=1e-6, stop_iteration=3e4):
    """Fractional step method for solving 3D Heat equation"""
    # U[z, y, x] or U[k, j, i]
    U_old = U.copy()
    ...
    while maximum > eps and iteration < stop_iteration:
        # Finding U^(n+1/3)
        A[0:P, 0:M, 0:N] = -a2 / (2*dx**2)
        B[0:P, 0:M, 0:N] = 1 / dt + a2 / dx**2
        C[0:P, 0:M, 0:N] = -a2 / (2*dx**2)
        
        D[1:P-1, 1:M-1, 1:N-1] = U_old[1:P-1, 1:M-1, 1:N-1] / dt \
            + a2*(
                (U_old[1:P-1, 1:M-1, 2:N] - 2*U_old[1:P-1, 1:M-1, 1:N-1] \
                    + U_old[1:P-1, 1:M-1, 0:N-2]) \
                        / (2*dx**2) \
        ...
        # Finding U^(n+2/3)
        A[0:P, 0:M, 0:N] = -a2 / (2*dy**2)
        B[0:P, 0:M, 0:N] = 1 / dt + a2 / dy**2
        C[0:P, 0:M, 0:N] = -a2 / (2*dy**2)
        ...
        # Finding U^(n+1)
        A[0:P, 0:M, 0:N] = -a2 / (2*dz**2)
        B[0:P, 0:M, 0:N] = 1 / dt + a2 / dz**2
        C[0:P, 0:M, 0:N] = -a2 / (2*dz**2)
...
@timeit
@jit(nopython=True)
def Numba_Fractional_step_method(U:np.ndarray, N, M, P, dx, dy, dz, dt, 
                                    a2=1, eps=1e-6, stop_iteration=3e4):
    """Boosted Fractional step method for solving 3D Heat equation"""
    # U[z, y, x] or U[k, j, i]
    U_old = U.copy()
    U_new = np.zeros_like(U)
...
U_1 = Fractional_step_method(U_old, N, M, P, dx, dy, dz, dt, a2=a2)

U_1N = Numba_Fractional_step_method(U_old, N, M, P, dx, dy, dz, dt, a2=a2)
U_1N = Numba_Fractional_step_method(U_old, N, M, P, dx, dy, dz, dt, a2=a2)
U_1N = Numba_Fractional_step_method(U_old, N, M, P, dx, dy, dz, dt, a2=a2)
