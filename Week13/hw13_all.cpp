...
// dt = 0.01, N = 101, M = 101
// Explcit + Jacobi = 25442 iterations
// Explcit + Gauss-Seidel = 25455 iterations
// Explcit + Relaxation = 25451 iterations

// dt = 0.01, N = 101, M = 101
// FSM + Jacobi = 509 iterations
// FSM + Gauss-Seidel = 498 iterations
// FSM + Relaxation = 506 iterations

// dt = 0.01, N = 101, M = 101
// ADM + Jacobi = 300 iterations
// ADM + Gauss-Seidel = 295 iterations
// ADM + Relaxation = 330 iterations
...
// Explicit scheme | Explicit method
...
// Fractional step method | Implicit method
...
// Alternating direction method | Implicit method
void ADM_Burger(double S_new[M][N], double S_old[M][N], double U_old[M][N], double V_old[M][N], 
                    int N, int M, double dx, double dy, double dt, double nu){
    double A[M][N], B[M][N], C[M][N], D[M][N];
    double alpha[M][N], beta[M][N];
    ...
    for(int j = 0; j < M; ++j){
        for(int i = 0; i < N; ++i){
            A[j][i] = -nu / (dx*dx) + U_old[j][i] / (dx);
            B[j][i] = 1 / dt + 2*nu / (dx*dx) - U_old[j][i] / (dx);
            C[j][i] = -nu / (dx*dx);
        }
    }
    ...
    for(int j = 0; j < M; ++j){
        for(int i = 0; i < N; ++i){
            A[j][i] = -nu / (dy*dy) + V_old[j][i] / (dy);
            B[j][i] = 1 / dt + 2*nu / (dy*dy) - V_old[j][i] / (dy);
            C[j][i] = -nu / (dy*dy);
        }
    }
    ...
    for(int j = 1; j < M-1; ++j){
        for(int i = 1; i < N-1; ++i){
            D[j][i] = S_new[j][i] / dt
                + nu*(S_new[j][i+1] - 2*S_new[j][i] + S_new[j][i-1])
                    / (dx*dx)
                - U_old[j][i] * (S_new[j][i+1] - S_new[j][i])
                    / dx;
        }
    }
...
// Gauss-Seidel method | Explicit method
...
// Relaxation method | Explicit method
void Relaxation_Poisson(double P_new[M][N], double P_old[M][N], double U_new[M][N], double V_new[M][N], 
                            int N, int M, double dx, double dy, double dt, double rho, double w=1.5){
    for(int j = 1; j < M-1; ++j){
        for(int i = 1; i < N-1; ++i){
            // central scheme
            P_new[j][i] = (1 - w)*P_old[j][i] + w*(
                dy*dy*(P_old[j][i+1] + P_new[j][i-1])
                + dx*dx*(P_old[j+1][i] + P_new[j-1][i])) 
                    / (2*(dx*dx + dy*dy))
                - w*dx*dx*dy*dy*rho
...
int main(){
    ...
    do{
        ...
        // 1.2 method
        // ADM_Burger(U_new, U_old, U_old, V_old, N, M, dx, dy, dt, nu);
        // ADM_Burger(V_new, V_old, U_old, V_old, N, M, dx, dy, dt, nu);
        ...
        do{
            ...
            // 2.3 method
            Relaxation_Poisson(P_new, P_old, U_new, V_new, N, M, dx, dy, dt, rho, 1.94);
