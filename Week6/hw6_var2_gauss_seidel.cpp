...
void Gauss_Seidel_method(double P_old[M][N], double P_new[M][N], int N, int M, 
                    double dx, double dy, double eps=1e-6, int stop_iteration=3e4){
    ...
    do{
        for(int j = 1; j < M-1; ++j){
            for(int i = 1; i < N-1; ++i){
                P_new[j][i] = (dy*dy*(
                    P_old[j][i+1] + P_new[j][i-1]
                    ) + dx*dx*(
                    P_old[j+1][i] + P_new[j-1][i]
                    )) / (2*(dx*dx + dy*dy));
            }
        }
...
int main(){
    ...
    Gauss_Seidel_method(P_old, P_new, N, M, dx, dy);
    ...
    savetxt("Results\\HW6_P2_cpp.txt", P_new, "%.6f", '\t');
